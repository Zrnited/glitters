...
export default function Navbar({searchbar, logoutBar, showLogoutBar, showSearchbar}: NavProps) {
  return (
    <header>
      <nav className="m-3 p-3 border border-[#2E2729] flex flex-row justify-between items-center lg:px-8">
        ...
        <div className="hidden sm:flex flex-row gap-x-5 md:gap-x-8 lg:gap-x-10">
          <div className="relative">
            ...
            />
            {searchbar && (<div className="absolute top-8 -left-24">
              <input type="text" className="bg-white pl-3 pr-10 border border-black h-[50px] focus:outline-none" placeholder="Enter the item name" />
              <Image alt="icon" priority src={search} className="absolute w-[22px] h-[22px] right-3 bottom-4 cursor-pointer" />
            </div>)}
          </div>
          <Link href={'/cart'}>
            <Image
          ...
          <div className="relative">
            ...
            />
            {logoutBar && (<div className="bg-white absolute flex flex-row gap-x-3 px-4 items-center border border-black h-[50px] w-[115px] top-8 -left-20 cursor-pointer">
              <p className="text-[#FF0C10]">Logout</p>
              <Image src={logout} alt="icon" priority className="w-[24px] h-[24px]" />
            </div>)}
          </div>
        </div>
